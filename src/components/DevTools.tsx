...
export const ChangeLogger = ({limit = 20}: ChangeLoggerProps) => {
	...
	useEffect(() => {
		const prev = store.getState().onNodesChange;
		store.setState({
			onNodesChange: (newChanges) => {
				handleNodeChanges(newChanges);
				prev?.(newChanges);
			},
		});
		return () => store.setState({onNodesChange: prev});
			...
			<div className='flex gap-1'>
				{tools.map(({active, setActive, label, value}) => (
					<Toggle
						...
						pressed={active}
						aria-pressed={active}
						onPressedChange={() => setActive((prev) => !prev)}
						size='sm'
						variant='outline'
					>
						{label}
...
export const DevTools = ({position}: DevToolsProps) => {
	...
	return (
		<>
			...
			{changeLoggerActive && (
				<Panel
					className='mt-20 max-h-[50%] overflow-y-auto text-xs text-muted-foreground bg-background/30 backdrop-blur-sm p-2 rounded border border-border'
			...
			{viewportLoggerActive && (
				<Panel position='bottom-left' className='text-muted-foreground text-xs'>
					<ViewportLogger />
				</Panel>
			)}
		</>
